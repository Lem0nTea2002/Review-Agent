...
from .ocr import OpenCodeReviewError
    ...
    def do_GET(self) -> None:
        ...
        if path == "/health":
            self._send_json(200, {"status": "ok", "reviewer": self.service.reviewer.name,
                                  ...
                                  "workspace_projects": self.service.projects.aliases,
                                  "ocr": self.service.ocr_status()})
            return
        ...
        if path == "/api/deployments/llm-review":
            ...
            return
        if path == "/v1/reviewers/ocr/status":
            self._send_json(200, self.service.ocr_status(check_version=True))
            return
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            if path == "/v1/reviews":
                ...
                return
            if path == "/v1/reviews/workspace":
                principal = self._principal("review")
                payload = self._read_json(body)
                request = WorkspaceReviewRequest.from_dict(payload)
                if request.background:
                    result = self.service.enqueue_workspace_review(
                        request, tenant_id=principal.tenant_id
                    )
                    status = 202
                else:
                    result = self.service.create_workspace_review(
                        request, tenant_id=principal.tenant_id
                    )
                    status = 201
                self.service.store.audit(
                    principal.tenant_id,
                    principal.username,
                    "workspace_review.create",
                    request.project,
                    {"target": request.target, "reviewers": list(request.reviewers),
            ...
            self._send_json(503, {"error": exc.code, "detail": str(exc)})
